...
describe('runAction', () => {
	...
	test('executes successful workflow with minimal setup', async () => {
		...
		// Verify logging
		expect(
			logger.hasMessage('info', '📊 Kover Coverage Report Action'),
		).toBe(true);
	...
	test('logs startup information correctly', async () => {
		...
		expect(logger.hasMessage('info', 'Minimum coverage requirement: 80%')).toBe(
			true,
		);
		expect(logger.hasMessage('info', 'Report title: Custom Coverage Report')).toBe(
			true,
		);
	});
	...
	test('logs ignored modules when configured', async () => {
		const logger = new SpyLogger();
		const discovery = createFakeDiscovery([
			{ name: ':app', filePath: 'app/report.xml' },
		]);
		const { reporter } = createSpyReporter();
		...
		expect(logger.hasMessage('info', 'Ignoring 2 modules: :test, :fixtures')).toBe(
			true,
		);
	});
	...
	test('handles discovery errors gracefully', async () => {
		...
		expect(logger.hasMessage('error', 'Action failed: Discovery failed')).toBe(
			true,
		);
	});
	...
	test('integrates with history manager when enabled', async () => {
		...
		const { reporter } = createSpyReporter();
		const { manager, getLoadCalled, getPersistCalled } =
			createFakeHistoryManager();
		...
		expect(
			logger.hasMessage(
				'info',
				'History tracking enabled (baseline: develop, retention: 100)',
			),
		).toBe(true);
	...
	test('handles history errors without failing the action', async () => {
		...
		expect(
			logger.hasMessage('warn', 'Failed to process coverage history'),
		).toBe(true);
	...
	test('logs history comparison when baseline exists', async () => {
		...
		expect(
			logger.hasMessage('info', 'Comparing with baseline (2025-01-01T00:00:00Z)'),
		).toBe(true);
	...
	test('logs warning when no baseline found', async () => {
		...
		expect(
			logger.hasMessage('info', 'No baseline found for branch: production'),
		).toBe(true);
	});
	...
	test('discovery mode is logged correctly', async () => {
		...
		expect(logger.hasMessage('info', 'Discovering modules (command mode)')).toBe(
			true,
		);
	});
	...
	test('works without history manager when history disabled', async () => {
		...
		expect(result.success).toBe(true);
		expect(
			logger.hasMessage('info', 'History tracking enabled'),
		).toBe(false);
	});
});
